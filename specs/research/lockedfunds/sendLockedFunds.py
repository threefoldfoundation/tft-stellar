...
def set_account_signers(address, public_key_signer,preauth_tx_hash, signer_kp):
    ...
    print(response)
    print('Set the signers of {address} to {pk_signer} and {preauth_hash_signer}'.format(
        address=address,
...
def send_locked_funds(from_secret,destination,asset,amount):
    ...
    print('Sending {amount} {asset} from {from_address} to {destination}'.format(amount=amount,asset=asset,from_address=from_kp.public_key,destination=destination))
