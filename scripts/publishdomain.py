...
@click.command(help='Set the homedomain on a custom asset as described at https://www.stellar.org/developers/guides/issuing-assets.html#discoverablity-and-meta-information ')
...
def set_home_domain(tokencode,home_domain,issuer_secret, network):
    ...
    print('Setting the homedomain for {tokencode}:{issuer} to {homedomain} on the {network} network'.format(
        tokencode=tokencode,
    ...
    transaction= stellar_sdk.transaction_builder.TransactionBuilder(
        issuer_account,
        network_passphrase=stellar_sdk.Network.TESTNET_NETWORK_PASSPHRASE if network=='test' else stellar_sdk.Network.PUBLIC_NETWORK_PASSPHRASE,
        base_fee=base_fee
