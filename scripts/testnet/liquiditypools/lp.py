...
@click.command(
    help="Manage testnet liquidity pools"
)
...
def manage_liquidity_pool_command(asset_pair,account_secret):
    ...
    for balance in account.raw_data["balances"]:
        if balance["asset_type"]=="liquidity_pool_shares" and balance["liquidity_pool_id"]== pool_asset.liquidity_pool_id:
            has_trustline = True
            break
    ...
    if Decimal(lp["total_shares"]).is_zero():
        ...
        tx = stellar_sdk.TransactionBuilder(account).append_liquidity_pool_deposit_op(
           pool_asset.liquidity_pool_id,
           max_amount_a,max_amount_b,min_price=min_price,max_price=max_price).build() 
        tx.sign(account_keypair)
        horizon_server.submit_transaction(tx)
    else:
